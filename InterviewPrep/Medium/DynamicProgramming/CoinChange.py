...
# top-down: recursive dfs, for amount, branch for each coin, cache to store prev coin_count
...
# dp[5] = 1 + dp[5 - 1] = 1 + dp[4] = 2
# dp[5] = 1 + dp[5 - 3] = 1 + dp[2] = 3
# dp[5] = 1 + dp[5 - 4] = 1 + dp[1] = 2
# dp[5] = 1 + dp[5 - 5] = 1 + dp[0] = 1
#   So the min for dp[5] = 1
# dp[6] = 1 + dp[6 - 1] = 1 + dp[5] = 2
# dp[6] = 1 + dp[6 - 3] = 1 + dp[3] = 2
# dp[6] = 1 + dp[6 - 4] = 1 + dp[2] = 3
# dp[6] = 1 + dp[6 - 5] = 1 + dp[1] = 2
#   So the min for dp[6] = 2
# dp[7] = 1 + dp[7 - 1] = 1 + dp[6] = 3
# dp[7] = 1 + dp[7 - 3] = 1 + dp[4] = 2
# dp[7] = 1 + dp[7 - 4] = 1 + dp[3] = 2
# dp[7] = 1 + dp[7 - 5] = 1 + dp[2] = 3
...
class Solution:
    def coinChange(self, coins: List[int], amount: int) -> int:
        ...
        print(f"dp: {dp}")
        dp[0] = 0 # base case for any target

        for a in range(1, amount + 1): # O(amount) loop
            print(f"a: {a} | dp: {dp}")
            for c in coins: # O(len(coins)) loop
                print(f"c: {c} | a-c: {a-c} | dp: {dp}") # this might cause problems for large inputs
                if a - c >=0:
